...
# Get flavor ID - try different API endpoints
echo "Finding flavor ID for $FLAVOR..."
FLAVOR_RESP=$(ovh_call GET "/cloud/project/$PROJECT_ID/flavor?region=$REGION" 2>/dev/null || echo "[]")

# Debug: show first few flavors
echo "Available flavors in $REGION:"
echo "$FLAVOR_RESP" | jq -r '.[] | "\(.name): \(.id)"' 2>/dev/null | head -10 || echo "(could not parse flavors)"
