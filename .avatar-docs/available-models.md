...
| Cute dog / animal source conditioning | `model_google-gemini-3-1-flash`, then `model_hunyuan-3d-pro-3-1-i23d` | Make the dog cute before conversion. Hunyuan 3D Pro mostly infers from image pixels; it is the conversion model, not the art-direction model. Avoid talking-portrait and human-face language in 3D prompts. |
| Cute dog / animal 3D fallbacks | `model_tripo-v3-0-image-to-3d`, `model_trellis-2`, `model_meta-sam-3d-objects` | Tripo gives explicit topology/face-limit/quad controls; Trellis gives remesh controls for organic shape; SAM3D is a simple centered-object fallback. Use these only after the source image is cute. |
| Image-conditioned 3D refiner | `model_ultrashape-1-0` | Requires both the original image and the coarse 3D model. Use when Hunyuan/Tripo gets the broad dog shape but loses cuteness, muzzle/eye personality, or source likeness. First-pass settings: `numInferenceSteps: 50`, `octreeResolution: 1024`, fixed seed. |
| Image-conditioned retexture | `model_trellis-2-retexture` | Requires both the original image and a 3D model. Use after geometry is accepted to recover fur texture, eye charm, colors, and markings. First-pass settings: `resolution: 1024`, `textureSize: 4096`, `texSlatGuidanceStrength: 1`, fixed seed. |
...
| Cute animal source conditioning | Scenario Gemini 3.1 Flash | P-Image Edit / Qwen Edit | Lock charm, dog anatomy, and non-human proportions before any 3D conversion. |
| Primary head/asset generation | Scenario Hunyuan 3D 3.1 Pro | Scenario Tripo P1 / Tripo 3.1 / Tripo 3.0 | Hosted first; compare high-fidelity and game-ready outputs in parallel. For dog assets, make source cuteness a hard precondition. |
| Likeness refinement | Scenario Ultrashape 1.0 | Trellis 2 Retexture | Use original image + generated GLB before retopo/rigging if the dog is close but loses cuteness/personality. |
