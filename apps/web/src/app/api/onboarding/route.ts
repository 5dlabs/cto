...
function processTransition(
  ...
  switch (currentState.state) {
    ...
    case "github_connect":
      if (action.action === "github_connected") {
        ...
        return {
          ...
          message:
            "GitHub connected! Which repositories should our agents work on?",
          actions: [
            {
              action_type: "select",
    ...
    case "agent_selection":
      if (
        action.action === "select_agents" ||
        action.action === "use_recommended"
      ) {
        return {
          state: {
            state: "provisioning",
          ...
          message:
            "Setting up your workspace... Creating isolated environment.",
          actions: [],
          progress: 85,
        };
      }
