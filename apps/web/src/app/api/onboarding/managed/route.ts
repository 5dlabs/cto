...
const CLUSTER_CONFIGS: Record<
  string,
  { nodes: number; cpNodes: number; workerNodes: number; plan: string }
> = {
...
export async function POST(request: NextRequest) {
  ...
  if (action === 'provision') {
    ...
    if (!clusterConfig) {
      return NextResponse.json({ error: `Invalid cluster size: ${size}` }, { status: 400 });
    }
    ...
    console.log(
      `[Managed Onboarding] Creating BoltRun for ${tenantId}:`,
      JSON.stringify(boltRunSpec, null, 2)
    );
    ...
    if (k8sApiServer && k8sToken) {
      try {
        ...
        return NextResponse.json({ error: 'Failed to create provisioning task' }, { status: 500 });
      }
    }
