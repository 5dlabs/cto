      ...
      return NextResponse.json(
        {
          ...
          message:
            'GitHub OAuth credentials are not set. Please configure GITHUB_CLIENT_ID and GITHUB_CLIENT_SECRET environment variables.',
        },
        { status: 500 }
      );
    }
  ...
  try {
    ...
    if (request.url.includes('/sign-in/social')) {
      if (!isOAuthConfigured()) {
        console.error(
          '[Auth POST] Social sign-in requested but GitHub OAuth credentials are not configured',
          {
        ...
        return NextResponse.json(
          {
            error: 'OAuth not configured',
            message:
              'GitHub OAuth is not configured. Please ensure GITHUB_CLIENT_ID and GITHUB_CLIENT_SECRET environment variables are set.',
