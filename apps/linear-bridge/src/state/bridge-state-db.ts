import { mkdirSync, existsSync, readFileSync, renameSync, unlinkSync } from "node:fs";
...
export interface BridgeStateDb {
  ...
  upsertRun(data: Omit<PersistedRun, "registeredAt" | "lastAccessedAt">): void;
  updateRun(runId: string, patch: Partial<Pick<PersistedRun, "linearSessionId" | "resumeToken">>): void;
  getRun(runId: string): PersistedRun | undefined;
  deleteRun(runId: string): void;
  gcRuns(maxAgeMs: number): number;
  countRuns(): number;
  listRuns(): Array<Omit<PersistedRun, "resumeToken" | "registeredAt" | "lastAccessedAt">>;
...
export function createBridgeStateDb(path: string): BridgeStateDb {
  ...
  const upsertRunStmt = db.prepare(`
    INSERT INTO runs(run_id, agent_pod, session_key, issue_id, linear_session_id, resume_token, registered_at, last_accessed_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(run_id) DO UPDATE SET
      agent_pod=excluded.agent_pod,
  ...
  const getRunStmt = db.prepare(`
    SELECT run_id, agent_pod, session_key, issue_id, linear_session_id, resume_token, registered_at, last_accessed_at
    FROM runs WHERE run_id = ?
  `);
  const updateRunStmt = db.prepare(`
    UPDATE runs
    SET linear_session_id = COALESCE(?, linear_session_id),
        resume_token = COALESCE(?, resume_token),
        last_accessed_at = ?
    WHERE run_id = ?
  `);
  const deleteRunStmt = db.prepare(`DELETE FROM runs WHERE run_id = ?`);
  const gcRunsStmt = db.prepare(`DELETE FROM runs WHERE last_accessed_at < ?`);
  const countRunsStmt = db.prepare(`SELECT COUNT(*) as count FROM runs`);
  const listRunsStmt = db.prepare(`
    SELECT run_id, agent_pod, session_key, issue_id, linear_session_id
    FROM runs
  ...
  return {
    ...
    updateRun(runId, patch) {
      updateRunStmt.run(patch.linearSessionId ?? null, patch.resumeToken ?? null, nowMs(), runId);
    },
    getRun(runId) {
    ...
    gcRuns(maxAgeMs) {
      const before = countRunsStmt.get() as { count: number };
      gcRunsStmt.run(nowMs() - maxAgeMs);
      const after = countRunsStmt.get() as { count: number };
      return before.count - after.count;
    },
    countRuns() {
      const row = countRunsStmt.get() as { count: number };
      return row.count;
    },
    listRuns() {
      const rows = listRunsStmt.all() as Array<{
        run_id: string;
        agent_pod: string;
        session_key: string;
        issue_id: string;
      ...
      return rows.map((r) => ({
        runId: r.run_id,
        agentPod: r.agent_pod,
        sessionKey: r.session_key,
        issueId: r.issue_id,
        linearSessionId: r.linear_session_id ?? undefined,
      }));
    },
...
export function removeLegacyStateFile(path: string): void {
  if (!existsSync(path)) return;
  try {
    unlinkSync(path);
  } catch {
