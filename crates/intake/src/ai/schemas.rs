...
use crate::entities::{ConstraintType, DecisionCategory, SubagentType, TaskPriority, TaskStatus};
...
/// A decision point generated by AI during PRD parsing.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct GeneratedDecisionPoint {
    /// Decision point ID (e.g., "d1", "d2")
    pub id: String,
    /// Category of decision
    pub category: DecisionCategory,
    /// Description of what needs to be decided
    pub description: String,
    /// Known options to consider
    #[serde(default)]
...
pub struct GeneratedTask {
    ...
    /// Decision points where agent judgment is required
    #[serde(default, rename = "decisionPoints")]
    pub decision_points: Vec<GeneratedDecisionPoint>,
}
...
mod tests {
    ...
    #[test]
    fn test_generated_task_with_decision_points() {
        let json = r#"{
            "id": 1,
            "title": "Setup Backend API",
            "description": "Implement the core API service",
            "details": "Create Axum router",
            "testStrategy": "Unit tests",
            "priority": "high",
            "dependencies": [],
            "decisionPoints": [
                ...
                },
                {
                    ...
                    "description": "Empty state design",
        ...
        let task: GeneratedTask = serde_json::from_str(json).unwrap();
        assert_eq!(task.decision_points.len(), 2);
        assert_eq!(task.decision_points[0].id, "d1");
        assert_eq!(
            task.decision_points[0].category,
            DecisionCategory::ErrorHandling
        );
        assert!(!task.decision_points[0].requires_approval);
        assert!(task.decision_points[1].requires_approval);
    }

    #[test]
    fn test_generated_task_without_decision_points() {
        // Should work without decision points (backwards compatible)
        let json = r#"{
            "id": 1,
            "title": "Simple task",
