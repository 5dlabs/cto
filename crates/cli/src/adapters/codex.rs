...
impl CodexAdapter {
    ...
    fn render_config(&self, context: &Value) -> AdapterResult<String> {
        ...
        let approval_policy = context["approval_policy"].as_str().unwrap_or("on-failure");
        let sandbox_mode = context["sandbox_mode"]
            .as_str()
            .unwrap_or("workspace-write");
        let project_doc_max_bytes = context["project_doc_max_bytes"].as_u64().unwrap_or(32768);

        let mut toml = format!(
            r#"# Codex CLI Configuration
...
"#
        ...
        if let Some(provider) = context["model_provider"].as_object() {
            let name = provider
                .get("name")
                .and_then(|v| v.as_str())
                .unwrap_or("OpenAI");
...
impl CliAdapter for CodexAdapter {
    ...
    async fn initialize(&self, container: &ContainerContext) -> Result<()> {
        info!(container_name = %container.container_name, "Initializing Codex adapter for container");
    ...
    async fn cleanup(&self, container: &ContainerContext) -> Result<()> {
        info!(container_name = %container.container_name, "Cleaning up Codex adapter");
