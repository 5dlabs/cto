...
//!
//! Core abstraction layer providing unified CLI interactions across 9 different CLI tools.
//! This trait system enables consistent behavior while preserving each CLI's unique capabilities.

use crate::types::{CLIExecutionContext, CLIExecutionResult, CLIType, ConfigFile};
use anyhow::Result;
...
impl CLIExecutionAdapter {
    ...
    /// Get the CLI type for this adapter
    #[must_use]
...
impl CommandBuilder {
    ...
    /// Get the CLI type for this builder
    #[must_use]
    pub fn cli_type(&self) -> CLIType {
        self.cli_type
    }
...
mod tests {
    ...
    fn test_command_builder_claude() {
        ...
        assert_eq!(cmd, vec!["claude", "-p", "implement auth"]);
    }
