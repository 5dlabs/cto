...
use metal::providers::{latitude::Latitude, ProviderError};
use tracing::{error, info, warn};

use crate::bare_metal::{get_latitude_credentials, BareMetalOrchestrator};
...
struct VlanAssignmentContext<'a> {
    vlan_id: &'a str,
...
impl Installer {
    ...
    fn warn_protected_field_changes(old: &InstallConfig, new: &InstallConfig) -> Result<()> {
        ...
        if let (Some(old_primary), Some(new_primary)) =
            (&old.primary_interface, &new.primary_interface)
        {
            if old_primary != new_primary {
                ui::print_warning(&format!(
                    "⚠️  Primary interface changed from '{old_primary}' to '{new_primary}' but servers already exist!"
                ));
                ui::print_info(
                    "   Primary interface is hardware-specific. Continuing with existing value.",
                );
            }
        }
    ...
    fn update_vlan_config(config: &mut InstallConfig, new_config: &InstallConfig) {
        ...
        if let Some(new_primary) = &new_config.primary_interface {
            if config.primary_interface.as_deref() != Some(new_primary) {
                ui::print_info(&format!(
                    "Updating primary interface: {} -> {}",
                    config.primary_interface.as_deref().unwrap_or("<unset>"),
                    new_primary
                ));
                config.primary_interface = Some(new_primary.clone());
            }
        }
    ...
    async fn execute_current_step(&mut self) -> Result<()> {
        ...
        match &self.state.step {
            ...
            InstallStep::AssigningVLAN => {
                self.assign_servers_to_vlan().await?;
            }
    ...
    /// Assign servers to VLAN via Latitude API.
    ///
    /// This tags the physical switch ports for VLAN traffic. The API assigns
    /// IPs via DHCP, but we configure Talos with our own static IPs.
    async fn assign_servers_to_vlan(&mut self) -> Result<()> {
        // Skip if VLAN is disabled
        if !self.state.config.enable_vlan {
            ui::print_info("VLAN disabled, skipping server assignment");
            return Ok(());
        }
        ...
        let vlan_id = self
            ...
            .vlan_id
            .as_ref()
            .context("VLAN ID not found - run CreatingVLAN step first")?
            .clone();
        ...
        let cp_id = self
            ...
            .control_plane
            .as_ref()
            .context("Control plane not found")?
            .id
            .clone();
        let worker_ids: Vec<String> = self.state.workers.iter().map(|w| w.id.clone()).collect();
        let expected_assignments = 1 + worker_ids.len();

        // Check existing assignments to avoid duplicates
        let (api_key, project_id) = get_latitude_credentials()?;
        let latitude =
            Latitude::new(&api_key, &project_id).context("Failed to create Latitude client")?;

        let context = VlanAssignmentContext {
            vlan_id: &vlan_id,
        ...
        let Some(assigned_servers) = self
            .list_vlan_assignments_or_wait(&latitude, &context)
            .await?
        else {
            return Ok(());
        };

        // Assign control plane if not already assigned
        if assigned_servers.contains(&cp_id) {
            ui::print_info("Control plane already assigned to VLAN");
        } else {
            let assigned = self
                .assign_server_to_vlan_or_wait(&latitude, &context, &cp_id, "control plane")
                .await?;
            if !assigned {
                return Ok(());
            }
        ...
        // Assign workers
        for (i, worker_id) in worker_ids.iter().enumerate() {
            if assigned_servers.contains(worker_id) {
                ui::print_info(&format!("Worker {i} already assigned to VLAN"));
            } else {
                let label = format!("worker {i}");
                let assigned = self
                    .assign_server_to_vlan_or_wait(&latitude, &context, worker_id, &label)
                    .await?;
                if !assigned {
                    return Ok(());
                }
        ...
        ui::print_success(&format!(
            "All {} servers assigned to VLAN {vlan_id}",
            1 + worker_ids.len()
        ));
    ...
    async fn list_vlan_assignments_or_wait(
        &self,
        latitude: &Latitude,
        context: &VlanAssignmentContext<'_>,
    ) -> Result<Option<HashSet<String>>> {
        let existing = match latitude.list_vlan_assignments(context.vlan_id).await {
            Ok(existing) => existing,
            Err(err) if Self::is_vlan_assignment_unavailable(&err) => {
                Self::warn_vlan_assignment_unavailable(context.vlan_id, &err);
                self.wait_for_manual_vlan_assignments(latitude, context)
                    .await?;
                return Ok(None);
            }
        ...
        Ok(Some(
            existing
                .iter()
                .filter_map(|assignment| assignment.attributes.server_id.clone())
                .collect(),
        ))
    }

    async fn assign_server_to_vlan_or_wait(
        &self,
        latitude: &Latitude,
        context: &VlanAssignmentContext<'_>,
        server_id: &str,
        ...
        ui::print_info(&format!("Assigning {label} ({server_id}) to VLAN..."));

        let assignment = match latitude
            .assign_server_to_vlan(context.vlan_id, server_id)
            .await
        {
        ...
        if let Some(dhcp_ip) = &assignment.attributes.ip {
            ui::print_info(&format!(
                "  DHCP assigned: {dhcp_ip} (will use static IP instead)"
            ));
        }
        ui::print_success(&format!("{label} assigned to VLAN"));
        Ok(true)
    }

    async fn wait_for_manual_vlan_assignments(
        &self,
        latitude: &Latitude,
        context: &VlanAssignmentContext<'_>,
    ) -> Result<()> {
        ...
        let expected_assignments = context.expected_assignments;
        let vlan_id = context.vlan_id;

        ui::print_warning("Waiting for manual VLAN assignment to complete.");
        ui::print_info(&format!("VLAN: {vlan_id}"));
        ui::print_info(&format!("Control plane server: {}", context.cp_id));
        for (i, worker_id) in context.worker_ids.iter().enumerate() {
            ui::print_info(&format!("Worker {i} server: {worker_id}"));
        }
        ui::print_info("Assign servers via Latitude dashboard or MCP private-networks-assign.");

        let start = Instant::now();
        loop {
            let vlan = latitude
                .get_virtual_network(vlan_id)
                .await
                .context("Failed to fetch VLAN assignments status")?;
            let assignments = vlan.attributes.assignments_count.unwrap_or(0) as usize;

            if assignments >= expected_assignments {
                ui::print_success(&format!(
                    "Detected {assignments}/{expected_assignments} VLAN assignments - continuing"
                ));
                return Ok(());
            }
            ...
            if start.elapsed() > Duration::from_secs(TIMEOUT_SECS) {
                anyhow::bail!(
                    "VLAN assignments missing after {TIMEOUT_SECS}s ({assignments}/{expected_assignments}). Assign servers to VLAN {vlan_id} and retry."
                );
            }

            ui::print_info(&format!(
                "Waiting for VLAN assignments ({assignments}/{expected_assignments})..."
            ));
    ...
    async fn wait_talos_maintenance(&mut self) -> Result<()> {
        ...
        // Auto-detect primary/secondary interfaces if enabled
        // This must happen while Talos is in maintenance mode (before config is applied)
        if self.state.config.enable_vlan {
            ui::print_info("Auto-detecting primary/secondary network interfaces...");
            match metal::talos::detect_network_interfaces(&cp_ip) {
                Ok(detected) => {
                    if detected.secondary == self.state.config.vlan_parent_interface {
                        ui::print_info(&format!(
                            "Detected VLAN interface '{}' matches default",
                            detected.secondary
                        ));
                    } else {
                        ui::print_info(&format!(
                            "Detected VLAN interface '{}' (overriding default '{}')",
                            detected.secondary, self.state.config.vlan_parent_interface
                        ));
                        self.state
                            .config
                            .vlan_parent_interface
                            .clone_from(&detected.secondary);
                    }

                    if self.state.config.primary_interface.as_deref()
                        == Some(detected.primary.as_str())
                    {
                        ui::print_info(&format!(
                            "Detected primary interface '{}' matches current",
                            detected.primary
                        ));
                    } else {
                        ui::print_info(&format!(
                            "Detected primary interface '{}' (updating)",
                            detected.primary
                        ));
                        self.state.config.primary_interface = Some(detected.primary);
                    }
                }
                Err(e) => {
                    ui::print_warning(&format!(
                        "Failed to auto-detect interfaces: {e}. Using defaults"
                    ));

                    if self.state.config.primary_interface.is_none() {
                        match metal::talos::detect_primary_interface(&cp_ip) {
                            Ok(primary) => {
                                ui::print_info(&format!(
                                    "Detected primary interface from addresses: '{primary}'"
                                ));
                                self.state.config.primary_interface = Some(primary);
                            }
                            Err(primary_err) => {
                                ui::print_warning(&format!(
                                    "Failed to detect primary interface from addresses: {primary_err}"
                                ));

                                if let Some(guess) = metal::talos::guess_primary_interface(
                                    &self.state.config.vlan_parent_interface,
                                ) {
                                    ui::print_warning(&format!(
                                        "Guessing primary interface '{guess}' from VLAN parent '{}'",
                                        self.state.config.vlan_parent_interface
                                    ));
                                    self.state.config.primary_interface = Some(guess);
                                }
                            }
                        }
                    }
    ...
    async fn generate_configs(&mut self) -> Result<()> {
        ...
        if self.state.config.enable_vlan && self.state.config.primary_interface.is_none() {
            if let Some(guess) =
                metal::talos::guess_primary_interface(&self.state.config.vlan_parent_interface)
            {
                ui::print_warning(&format!(
                    "Primary interface not detected; guessing '{guess}' from VLAN parent '{}'",
                    self.state.config.vlan_parent_interface
                ));
                self.state.config.primary_interface = Some(guess);
            }
            ...
            if let Some(vlan_vid) = self.state.vlan_vid {
                ...
                config = config
                    .with_vlan_interface(
                        ...
                        &private_ip_cidr,
                        &self.state.config.vlan_parent_interface,
                        self.state.config.primary_interface.as_deref(),
                    )
    ...
    async fn apply_worker_configs(&mut self) -> Result<()> {
        ...
        for (i, worker) in self.state.workers.iter().enumerate() {
            ...
            if let Some(vlan_vid) = vlan_vid {
                ...
                let (worker_interface, worker_primary_interface) =
                    match metal::talos::detect_network_interfaces(&worker.ip) {
                        Ok(interfaces) => {
                            ui::print_info(&format!(
                                "  Detected interfaces for worker: primary='{}', secondary='{}'",
                                interfaces.primary, interfaces.secondary
                            ));
                            (interfaces.secondary, Some(interfaces.primary))
                        }
                        Err(e) => {
                            // Fall back to the interfaces detected from CP
                            ui::print_warning(&format!(
                                "  Failed to detect interfaces: {}. Using defaults '{}'",
                                e, self.state.config.vlan_parent_interface
                            ));

                            let worker_interface = self.state.config.vlan_parent_interface.clone();
                            let worker_primary_interface =
                                metal::talos::detect_primary_interface(&worker.ip)
                                    .ok()
                                    .or_else(|| self.state.config.primary_interface.clone())
                                    .or_else(|| {
                                        metal::talos::guess_primary_interface(&worker_interface)
                                    });

                            if let Some(primary) = &worker_primary_interface {
                                ui::print_warning(&format!(
                                    "  Using primary interface fallback: '{primary}'"
                                ));
                            } else {
                                ui::print_warning(
                                    "  No primary interface detected; VLAN config may omit DHCP",
                                );
                            }

                            (worker_interface, worker_primary_interface)
                        }
                    };
                ...
                let vlan = metal::talos::VlanConfig {
                    ...
                    primary_interface: worker_primary_interface,
                };
...
";
    ...
    /// Check if VLAN assignment endpoints are unavailable on the provider API.
    fn is_vlan_assignment_unavailable(error: &ProviderError) -> bool {
        matches!(
            error,
    ...
    /// Warn and continue when VLAN assignment endpoints are unavailable.
    fn warn_vlan_assignment_unavailable(vlan_id: &str, error: &ProviderError) {
        ui::print_warning(&format!(
            "Latitude VLAN assignment API unavailable for {vlan_id}: {error}"
        ));
        ui::print_info(
            "API assignment unavailable. Waiting for manual VLAN assignment to complete.",
        );
        ui::print_info("Talos configs will still include VLAN interfaces with static IPs.");
    }
    ...
    fn print_success_summary(&self) {
        ...
        if let Some(ref kc) = self.state.kubeconfig_path {
            ui::print_section("Verifying Cluster Health...");
            match Self::verify_all_pods_healthy(kc) {
                Ok(()) => ui::print_success("All critical pods are healthy!"),
                Err(issues) => {
                    ui::print_warning("Some pods have issues (non-blocking):");
                    for issue in issues {
                        ui::print_info(&format!("  - {issue}"));
                    }
                }
            }
        }

        // Auto-open Lens as the final step to confirm completion
        ui::print_section("Opening Lens...");
        if kubeconfig::open_lens() {
            ui::print_success(&format!(
                "Look for cluster '{}' in the Lens catalog!",
                self.state.config.cluster_name
            ));
        } else {
            ui::print_info("Install Lens to visualize your cluster: brew install --cask openlens");
        }
    }

    /// Verify all pods in critical namespaces are healthy.
    ///
    /// Returns Ok(()) if all pods are healthy, Err with list of issues otherwise.
    fn verify_all_pods_healthy(kubeconfig: &Path) -> std::result::Result<(), Vec<String>> {
        use std::process::Command;

        let critical_namespaces = ["kube-system", "argocd", "cert-manager"];
                ...
                if output.status.success() {
                    ...
                    for line in stdout.lines() {
                        ...
                        if parts.len() >= 2 {
                            let name = parts[0];
                            let phase = parts[1];
                            let ready = parts.get(2).unwrap_or(&"");
                            ...
                            if phase != "Running" {
                                ...
                                // Known non-critical issues we can ignore
                                if name.contains("hubble-relay") {
                                    // Hubble relay often has mTLS issues, not critical
                                    continue;
                                }
                                issues.push(format!("{ns}/{name}: containers not ready"));
                            }
                        }
                    }
                }
