...
/// Open Lens (or OpenLens) if installed.
///
/// Returns true if Lens was successfully opened, false otherwise.
pub fn open_lens() -> bool {
    let lens = LensInstallation::detect();

    match lens {
        LensInstallation::LensDesktop => {
            ui::print_info("Opening Lens Desktop...");
            let result = Command::new("open").args(["-a", "Lens"]).spawn();
            if result.is_ok() {
                ui::print_success("Lens Desktop opened!");
                return true;
            }
        }
        LensInstallation::OpenLens => {
            ui::print_info("Opening OpenLens...");
            let result = Command::new("open").args(["-a", "OpenLens"]).spawn();
            if result.is_ok() {
                ui::print_success("OpenLens opened!");
                return true;
            }
        }
        LensInstallation::NotInstalled => {
            ui::print_warning("Lens/OpenLens not installed - cannot auto-open.");
        }
    }
    false
}
