...
impl GitOpsVerifier {
    ...
    pub async fn apply_app_of_apps(&self, repo: &str, branch: &str) -> Result<()> {
        ...
        // LESSON LEARNED: The ArgoCD CRDs can lag behind the deployment.
        // Wait for Application/AppProject CRDs before applying manifests.
        self.wait_for_argocd_crds(Duration::from_secs(120)).await?;
    ...
    async fn wait_for_argocd_crds(&self, timeout: Duration) -> Result<()> {
        let start = Instant::now();
        let crds = ["applications.argoproj.io", "appprojects.argoproj.io"];
        let api: Api<CustomResourceDefinition> = Api::all(self.client.clone());

        info!("Waiting for ArgoCD CRDs to be available...");

        loop {
            if start.elapsed() > timeout {
                anyhow::bail!("Timeout waiting for ArgoCD CRDs: {}", crds.join(", "));
            }

            let mut missing = Vec::new();
            for crd in &crds {
                match api.get(crd).await {
                    Ok(_) => {}
                    Err(kube::Error::Api(e)) if e.code == 404 => {
                        missing.push(*crd);
                    }
                    Err(e) => {
                        warn!(error = %e, crd = %crd, "Failed to check ArgoCD CRD");
                        missing.push(*crd);
                    }
                }
            }
