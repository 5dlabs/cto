...
fn parse_task_context(content: &str) -> TaskContext {
    ...
    for line in &lines {
        if line.contains("**Agent**:") {
            if let Some(agent_part) = line.split("**Agent**:").nth(1) {
                let agent = agent_part.split('|').next().map(|s| s.trim().to_string());
                ctx.agent = agent;
            }
        }
        if line.contains("**Language**:") {
            if let Some(lang_part) = line.split("**Language**:").nth(1) {
                let lang = lang_part.split('|').next().map(|s| s.trim().to_string());
                ctx.language = lang;
            }
        }
    }
    ...
    if let Some(start) = content.find("<requirements>") {
        if let Some(end) = content.find("</requirements>") {
            ...
            ctx.requirements = req_content
                .lines()
                .map(|l| l.trim())
                .filter(|l| l.starts_with('-') || l.starts_with('*'))
                .map(|l| {
                    l.trim_start_matches(|c| c == '-' || c == '*' || c == ' ')
                        .to_string()
                })
                .filter(|l| !l.is_empty())
...
fn narrate_tool_call(
    tool_name: &str,
    params: &serde_json::Value,
    state: &NarrativeState,
) -> Option<(NarrativePhase, String, bool)> {
    ...
    match tool_name {
        "Read" => {
            ...
            if file_path.contains("prd") || file_path.contains("PRD") {
                Some((
                    NarrativePhase::Understanding,
                    "Checking the PRD to understand what we're building...".to_string(),
                    true,
                ))
            } else if file_path.contains("architecture") || file_path.contains("ARCHITECTURE") {
                Some((
                    NarrativePhase::Understanding,
                    "Looking at the architecture to see how things fit together...".to_string(),
                    true,
                ))
            } else if file_path.contains("prompt.md") && !file_path.contains("subtask") {
                Some((
                    NarrativePhase::Understanding,
                    "Got the task brief, let me see what's needed...".to_string(),
                    true,
                ))
            } else if file_path.contains("acceptance") {
                Some((
                    NarrativePhase::Planning,
                    "Checking the acceptance criteria - need to know what 'done' looks like..."
                        .to_string(),
                    true,
                ))
            } else if file_path.contains("subtask") && file_path.contains("prompt") {
                // Extract subtask number from path
                let subtask_num = file_path
                    .split('/')
                    .rev()
                ...
                Some((
                    NarrativePhase::Planning,
                    format!("Looking at subtask {}...", subtask_num),
                    true,
                ))
            } else if file_path.contains("COMPLETION") || file_path.contains("SUMMARY") {
                Some((
                    NarrativePhase::Building,
                    "Checking what's already been done...".to_string(),
                    true,
                ))
        ...
        "Write" => {
            ...
            if file_path.contains("COMPLETION") || file_path.contains("SUMMARY") {
                Some((
                    NarrativePhase::Building,
                    format!("Writing up the completion report..."),
                    true,
                ))
            } else if file_path.ends_with(".yaml") || file_path.ends_with(".yml") {
                ...
                if file_path.contains("postgres") {
                    Some((
                        NarrativePhase::Building,
                        "Setting up PostgreSQL cluster config...".to_string(),
                        true,
                    ))
                } else if file_path.contains("redis") || file_path.contains("valkey") {
                    Some((
                        NarrativePhase::Building,
                        "Configuring Redis for caching...".to_string(),
                        true,
                    ))
                } else if file_path.contains("kafka") {
                    Some((
                        NarrativePhase::Building,
                        "Wiring up Kafka cluster...".to_string(),
                        true,
                    ))
                } else if file_path.contains("mongo") {
                    Some((
                        NarrativePhase::Building,
                        "Setting up MongoDB...".to_string(),
                        true,
                    ))
                } else if file_path.contains("rabbit") {
                    Some((
                        NarrativePhase::Building,
                        "Deploying RabbitMQ...".to_string(),
                        true,
                    ))
                } else {
                    Some((
                ...
                Some((
                    NarrativePhase::Building,
                    format!("Writing Go code: `{}`", filename),
                    true,
                ))
            } else if file_path.ends_with(".rs") {
                Some((
                    NarrativePhase::Building,
                    format!("Writing Rust: `{}`", filename),
                    true,
                ))
            } else if file_path.ends_with(".py") {
                Some((
                    NarrativePhase::Building,
                    format!("Writing Python: `{}`", filename),
                    true,
                ))
            } else if file_path.ends_with(".ts") || file_path.ends_with(".tsx") {
                Some((
                    NarrativePhase::Building,
                    format!("Writing TypeScript: `{}`", filename),
                    true,
                ))
            } else if file_path.contains("test") {
                Some((
                    NarrativePhase::Testing,
                    format!("Adding test: `{}`", filename),
                    true,
                ))
            } else {
                Some((
        ...
        "Bash" => {
            ...
            if command.contains("git push") {
                Some((
                    NarrativePhase::Shipping,
                    "Pushing changes to remote...".to_string(),
                    true,
                ))
                ...
                Some((
                    NarrativePhase::Shipping,
                    "Setting up the feature branch...".to_string(),
                    true,
                ))
            } else if command.contains("helm lint") {
                Some((
                    NarrativePhase::Testing,
                    "Validating Helm charts...".to_string(),
                    true,
                ))
            } else if command.contains("helm template") {
                Some((
                    NarrativePhase::Testing,
                    "Testing Helm template rendering...".to_string(),
                    true,
                ))
            } else if command.contains("kubectl") && command.contains("dry-run") {
                Some((
                    NarrativePhase::Testing,
                    "Dry-run testing K8s manifests...".to_string(),
                    true,
                ))
            } else if command.contains("yamllint") {
                Some((
                    NarrativePhase::Testing,
                    "Checking YAML formatting...".to_string(),
                    true,
                ))
        ...
        tool if tool.starts_with("mcp__") || tool.contains("github") => {
            if tool.contains("create_pull_request") {
                ...
                Some((
                    NarrativePhase::Shipping,
                    "🌿 Creating feature branch...".to_string(),
                    true,
                ))
...
async fn post_milestone_comment(
    state: &AppState,
    ...
    let body = match milestone_type {
        MilestoneType::TaskStarted => {
            ...
            let is_subagent = !["bolt", "rex", "morgan", "blaze", "agent"]
                .contains(&agent_name.to_lowercase().as_str());
            ...
            let mut msg = format!(
                ...
                prefix,
                agent_emoji,
                capitalize_agent_name(agent_name),
                title,
                details
            );
    ...
    if !status.is_success() {
        ...
        warn!(
            "Failed to post milestone comment ({}): {}",
...
async fn post_init_activity(
    ...
    if let Some(ref goal) = task.goal {
        let goal_short = if goal.len() > 150 {
            format!("{}...", &goal[..150])
        } else {
            goal.clone()
        };
    ...
    if !mcp_tools.is_empty() {
        ...
        let extras = if mcp_tools.len() > 5 {
            format!(" +{} more", mcp_tools.len() - 5)
        } else {
            String::new()
        };
        body.push_str(&format!(
    ...
    if !skills.is_empty() {
        ...
        let extras = if skills.len() > 5 {
            format!(" +{} more", skills.len() - 5)
        } else {
            String::new()
        };
        body.push_str(&format!(
    ...
    let action_line = if let Some(ref goal) = task.goal {
        ...
        } else if goal_lower.contains("kubernetes")
            || goal_lower.contains("k8s")
            || goal_lower.contains("infra")
        {
...
#[allow(dead_code)]
async fn post_conductor_update(state: &AppState, session_id: &str, message: &str) -> Result<()> {
    let client = reqwest::Client::new();
...
async fn post_completion_summary(
    ...
    let is_subagent =
        !["bolt", "rex", "morgan", "blaze", "agent"].contains(&agent_name.to_lowercase().as_str());
    let prefix = if is_subagent { "↳ " } else { "" };
    ...
    // Built-in tools used
    let builtin_used: Vec<_> = tools_used
        .iter()
        .filter(|t| !t.starts_with("mcp__"))
        .cloned()
        .collect();
    ...
    let signoff = if !files_created.is_empty() {
        ...
        if count == 1 {
            ...
            format!(
                "Done! {} files created, ready for review. {}",
                count, agent_emoji
            )
        }
...
fn process_tool_for_narrative(
    ...
    if let Some((new_phase, description, is_significant)) =
        narrate_tool_call(tool_name, params, narrative)
    {
...
async fn add_linear_activity(state: &AppState, session_id: &str, entry: &LogEntry) -> Result<()> {
    ...
    let content: Option<serde_json::Value> = match entry.entry_type.as_deref() {
        ...
        Some("tool_call") => {
            ...
            let (should_post, narrative, new_phase) =
                process_tool_for_narrative(tool_name, &params, &narrative_state);
        ...
        Some("tool_use") | Some("tool") => {
            ...
            let (should_post, narrative, new_phase) =
                process_tool_for_narrative(tool_name, &input, &narrative_state);
        ...
        Some("assistant") => {
            ...
            if let Some((tool_name, tool_input)) = extract_claude_tool_use(entry) {
                // Try narrative description
                let (should_post, narrative, new_phase) =
                    process_tool_for_narrative(&tool_name, &tool_input, &narrative_state);
...
async fn ingest(State(state): State<Arc<AppState>>, Json(entry): Json<LogEntry>) -> StatusCode {
    ...
    if is_init {
        ...
        info!(
            "🚀 Init event received: model={}, tools={}, skills={}",
            model,
            tools.len(),
            skills.len()
        );
        ...
        match create_linear_session(&state, &model, &tools, &skills).await {
            Ok(session_id) => {
                ...
                if let Err(e) =
                    post_init_activity(&state, &session_id, &model, &tools, &skills).await
                {
                ...
                if let Err(e) =
                    post_milestone_comment(&state, MilestoneType::TaskStarted, &goal_preview).await
                {
                ...
                match name {
                    "Write" | "write" => {
                        if let Some(file) = params
                            .and_then(|p| p.get("file_path").or(p.get("path")))
                            .and_then(|v| v.as_str())
                        {
                    ...
                    "Edit" | "edit" => {
                            ...
                            .and_then(|v| v.as_str())
                        {
                            ...
                            if !session.files_modified.contains(&file.to_string())
                                && !session.files_created.contains(&file.to_string())
                            {
    ...
    if entry.entry_type.as_deref() == Some("result") {
        if let Some(ref session_id) = session.session_id {
            ...
            let cost_usd = entry.extra.get("total_cost_usd").and_then(|v| v.as_f64());
...
async fn main() -> Result<()> {
    ...
    let linear_token = env::var("LINEAR_OAUTH_TOKEN").context("LINEAR_OAUTH_TOKEN not set")?;
    let issue_identifier =
        env::var("LINEAR_ISSUE_IDENTIFIER").context("LINEAR_ISSUE_IDENTIFIER not set")?;
    let agent_name = env::var("CTO_AGENT_NAME").unwrap_or_else(|_| "unknown".to_string());
    let log_source = env::var("LOG_SOURCE").unwrap_or_else(|_| "file".to_string());

    // Load workspace path and task context
    let workspace_path = env::var("WORKSPACE_PATH").map(PathBuf::from).ok();
        ...
        if prompt_path.exists() {
            match std::fs::read_to_string(&prompt_path) {
                Ok(content) => {
                    ...
                    info!(
                        "📋 Loaded task context: title={:?}, agent={:?}",
                        ctx.title, ctx.agent
                    );
                    ctx
                }
    ...
    if let Some(ref ws_path) = workspace_path {
        ...
        if acceptance_path.exists() {
            if let Ok(content) = std::fs::read_to_string(&acceptance_path) {
                ...
                info!(
                    "📊 Loaded acceptance criteria: {}/{} complete",
                    completed, total
                );
            }
        }
    ...
    info!(
        "Issue: {}, Agent: {}, Source: {}",
        issue_identifier, agent_name, log_source
    );
    ...
    match log_source.as_str() {
        ...
        "file" | _ => {
            ...
            let stream_file =
                env::var("STREAM_FILE").unwrap_or_else(|_| "/workspace/stream.jsonl".to_string());
