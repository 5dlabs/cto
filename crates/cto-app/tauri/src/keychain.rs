// OS Keychain integration for secrets
use keyring::Entry;
use serde::{Deserialize, Serialize};

fn entry(key: &str) -> Result<Entry, String> {
    Entry::new("cto-app", key)
        .map_err(|e| format!("Failed to create keychain entry '{key}': {e}"))
}

#[derive(Debug, Serialize, Deserialize)]
pub struct GitHubToken {
    pub access_token: String,
    pub expires_at: Option<u64>,
}

#[tauri::command]
pub fn get_password(key: &str) -> Result<Option<String>, String> {
    let entry = entry(key)?;

    match entry.get_password() {
        Ok(value) => Ok(Some(value)),
        Err(keyring::Error::NoEntry) => Ok(None),
        Err(e) => Err(format!("Failed to get '{key}': {e}")),
    }
}

#[tauri::command]
pub fn set_password(key: &str, value: &str) -> Result<(), String> {
    let entry = entry(key)?;

    entry
        .set_password(value)
        .map_err(|e| format!("Failed to set '{key}': {e}"))
}

#[tauri::command]
pub fn delete_password(key: &str) -> Result<(), String> {
    let entry = entry(key)?;

    entry
        .delete_password()
        .map_err(|e| format!("Failed to delete '{key}': {e}"))
}

#[tauri::command]
pub fn get_github_token() -> Result<Option<GitHubToken>, String> {
    let entry = entry("github_token")?;

    match entry.get_password() {
        Ok(token) => Ok(Some(GitHubToken {
            access_token: token,
            expires_at: None,
        })),
        Err(keyring::Error::NoEntry) => Ok(None),
        Err(e) => Err(format!("Failed to get GitHub token: {e}")),
    }
}

#[tauri::command]
pub fn set_github_token(token: &str) -> Result<(), String> {
    let entry = entry("github_token")?;

    entry
        .set_password(token)
        .map_err(|e| format!("Failed to save GitHub token: {e}"))
}

#[tauri::command]
pub fn delete_github_token() -> Result<(), String> {
    let entry = entry("github_token")?;

    entry
        .delete_password()
        .map_err(|e| format!("Failed to delete GitHub token: {e}"))
}

#[tauri::command]
pub fn get_cf_tunnel_token() -> Result<Option<String>, String> {
    let entry = entry("cf_tunnel_token")?;

    match entry.get_password() {
        Ok(token) => Ok(Some(token)),
        Err(keyring::Error::NoEntry) => Ok(None),
        Err(e) => Err(format!("Failed to get Cloudflare tunnel token: {e}")),
    }
}

#[tauri::command]
pub fn set_cf_tunnel_token(token: &str) -> Result<(), String> {
    let entry = entry("cf_tunnel_token")?;

    entry
        .set_password(token)
        .map_err(|e| format!("Failed to save Cloudflare tunnel token: {e}"))
}
