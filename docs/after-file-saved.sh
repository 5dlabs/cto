...
refresh_github_token_if_needed() {
    ...
    if [ -z "$GITHUB_APP_PRIVATE_KEY" ] || [ -z "$GITHUB_APP_ID" ]; then
        echo "[Auto-save] ⚠️ Missing GitHub App credentials, cannot refresh token"
        return 1
    fi
    ...
    if [ -z "$INSTALLATION_ID" ]; then
        ...
        REPO_URL="https://github.com/5dlabs/cto"
        ...
        if [ "$INSTALLATION_ID" = "null" ] || [ -z "$INSTALLATION_ID" ]; then
            echo "[Auto-save] ⚠️ Repo installation not found, trying org installation..."
        ...
        if [ "$INSTALLATION_ID" = "null" ] || [ -z "$INSTALLATION_ID" ]; then
            echo "[Auto-save] ❌ Failed to get installation ID"
    ...
    if [ -n "$NEW_TOKEN" ] && [ "$NEW_TOKEN" != "null" ]; then
        ...
        echo "[Auto-save] ❌ Failed to refresh token: $TOKEN_RESPONSE"
        rm -f "$TEMP_KEY"
        return 1
    fi
}
