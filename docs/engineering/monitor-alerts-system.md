...
| A5 | Post-Tess CI Failure | Poll `gh pr checks` after Tess approval | Tess approved but CI checks still failing | [A5 Prompt](#a5-post-tess-ci-failure) |
| A6 | Merge Conflict | Check PR mergeable status | `mergeable: false` - Atlas failed integration | [A6 Prompt](#a6-merge-conflict) |
| A7 | Pod Failure (Any CTO Pod) | Watch all pods in `agent-platform` namespace | Any pod enters Failed/Error/CrashLoopBackOff | [A7 Prompt](#a7-pod-failure) |
...
**Factory Prompt:**
```markdown
# Alert A5: Post-Tess CI Failure

## Detected Condition
- **Tess Approval Time**: {{tess_approval_time}}
- **CI Failures After Approval**: {{failed_check_count}}

This is a critical issue: Tess approved the PR but CI is still failing.
Either Tess approved incorrectly, or something broke after approval.
...
## Your Task
1. Analyze why Tess approved despite failing CI
2. Check if new commits after Tess approval broke the build
3. Determine if Tess needs to re-evaluate
4. Write analysis to `/workspace/watch/alerts/A5-{{timestamp}}.md`
```
...
**Detection Logic (Rust):**
```rust
fn check_merge_conflicts(pr_status: &PrStatus) -> Option<Alert> {
    if !pr_status.mergeable || pr_status.merge_state_status == "DIRTY" {
        return Some(Alert::A6(format!(
            "PR has merge conflicts - Atlas integration failure: {}",
...
**Factory Prompt:**
```markdown
# Alert A6: Merge Conflict (Atlas Integration Failure)
...
This indicates Atlas (Integration Agent) failed to properly integrate changes.
Atlas is responsible for ensuring PRs can merge cleanly.
...
## Your Task
1. Identify what caused the conflict (concurrent PR merged to main?)
2. Determine if Atlas should have caught this earlier
3. Check if Atlas attempted a rebase/merge and failed
4. Write analysis to `/workspace/watch/alerts/A6-{{timestamp}}.md`

Include:
- Root cause (race condition, missed dependency, etc.)
- Atlas's logs from the integration step
...
From this one response we evaluate: A1, A3, A4, A5, A6
...
| A5 | Post-Tess CI Failure | Inject Tess approval comment + `statusCheckRollup: FAILURE` | Alert A5 fires |
| A6 | Merge Conflict | Inject GitHub `mergeable: false` | Alert A6 fires |
...
# A5: Force post-Tess CI failure
gh pr comment 123 --body "✅ Tess approves"
# Then push a commit that breaks CI

# A6: Force merge conflict
git checkout main && echo "conflict" >> file.txt && git commit -am "conflict" && git push
# Now the PR has conflicts
...
play-monitor test-mode --mock-k8s --mock-github &
MONITOR_PID=$!

# Run test scenarios
for test in a1 a2 a3 a4 a5 a6 a7 a8 c1 c2 c3 c4 c5 c6; do
  echo "Testing $test..."
  play-monitor inject-test-scenario --scenario $test
  sleep 2
  
  if play-monitor check-alert-fired --alert $test; then
    echo "✅ $test: PASSED"
  else
    echo "❌ $test: FAILED"
    FAILED=1
