...
curl 'http://localhost:9428/select/logsql/query' \
  -d 'query={kubernetes_namespace="agent-platform"}' \
  -d 'limit=50'
```
...
```logql
# Single label match
{kubernetes_namespace="agent-platform"}

# Multiple labels (AND)
{kubernetes_namespace="agent-platform", app_kubernetes_io_name="agent-controller"}
...
```logql
# Contains text
{kubernetes_namespace="agent-platform"} |~ "error"

# Case-insensitive contains
{kubernetes_namespace="agent-platform"} |~ "(?i)error"

# Does not contain
{kubernetes_namespace="agent-platform"} !~ "debug"

# Exact match
{kubernetes_namespace="agent-platform"} | log == "Application started"
```
...
#### All Rex Agent Logs
```logql
{kubernetes_namespace="agent-platform", kubernetes_pod_name=~"rex-.*"}
```

#### Cleo Code Review Logs
```logql
{kubernetes_namespace="agent-platform", kubernetes_pod_name=~"cleo-.*"}
```

#### Tess QA Logs
```logql
{kubernetes_namespace="agent-platform", kubernetes_pod_name=~"tess-.*"}
```

#### All Agent Logs (Rex + Cleo + Tess)
```logql
{kubernetes_namespace="agent-platform", kubernetes_pod_name=~"(rex|cleo|tess)-.*"}
```

#### Logs for Specific Task
```logql
{kubernetes_namespace="agent-platform", kubernetes_pod_name=~"task-.*-task-123-.*"}
```

#### Agent Error Logs Only
```logql
{kubernetes_namespace="agent-platform", kubernetes_pod_name=~"(rex|cleo|tess)-.*"} |~ "(?i)(error|failed|failure)"
```
...
#### All Controller Logs
```logql
{kubernetes_namespace="agent-platform", kubernetes_pod_name=~"agent-controller-.*"}
```

#### Controller Errors
```logql
{kubernetes_namespace="agent-platform", kubernetes_pod_name=~"agent-controller-.*"} |~ "ERROR"
```

#### Controller Task Processing
```logql
{kubernetes_namespace="agent-platform", kubernetes_pod_name=~"agent-controller-.*"} |~ "Processing task"
```
...
#### Last Hour
```logql
{kubernetes_namespace="agent-platform"} | __timestamp__ >= now() - 1h
```

#### Specific Time Range
```logql
{kubernetes_namespace="agent-platform"} | __timestamp__ >= "2025-11-24T00:00:00Z" and __timestamp__ < "2025-11-24T23:59:59Z"
```

#### Today's Logs
```logql
{kubernetes_namespace="agent-platform"} | __timestamp__ >= today()
```
...
```logql
{kubernetes_namespace="agent-platform"} |~ "(?i)(pull request|PR)" |~ "created"
```

#### Find GitHub API Errors
```logql
{kubernetes_namespace="agent-platform"} |~ "github" |~ "(?i)(error|failed|4[0-9]{2}|5[0-9]{2})"
```

#### Find Long-Running Operations
```logql
{kubernetes_namespace="agent-platform"} |~ "duration" |~ "[0-9]+m"
```

#### Agent Startup/Shutdown
```logql
{kubernetes_namespace="agent-platform"} |~ "(?i)(starting|stopping|shutdown|terminated)"
```
...
{kubernetes_namespace="agent-platform"} | json | status_code >= 400

# Parse JSON and extract specific fields
{kubernetes_namespace="agent-platform"} | json | line_format "{{.timestamp}} [{{.level}}] {{.message}}"
```
...
### Count Logs
```logql
count_over_time({kubernetes_namespace="agent-platform"}[5m])
```

### Rate of Logs
```logql
rate({kubernetes_namespace="agent-platform"}[5m])
```

### Bytes per Second
```logql
bytes_over_time({kubernetes_namespace="agent-platform"}[5m])
```

### Count by Label
```logql
sum(count_over_time({kubernetes_namespace="agent-platform"}[1h])) by (kubernetes_pod_name)
```
...
2. Look for errors:
```logql
{kubernetes_namespace="agent-platform", kubernetes_pod_name=~"task-.*-task-123-.*"} |~ "(?i)error"
```

3. Check controller processing:
```logql
{kubernetes_namespace="agent-platform", kubernetes_pod_name=~"agent-controller-.*"} |~ "task-123"
```
...
2. Check for retries:
```logql
{kubernetes_namespace="agent-platform"} |~ "(?i)retr(y|ying|ied)"
```

3. Look for timeouts:
```logql
{kubernetes_namespace="agent-platform"} |~ "(?i)timeout"
```
...
```logql
{kubernetes_namespace="agent-platform"} |~ "github.com/api"
```

2. Check for rate limits:
```logql
{kubernetes_namespace="agent-platform"} |~ "rate limit"
```

3. Find authentication errors:
```logql
{kubernetes_namespace="agent-platform"} |~ "github" |~ "(?i)(unauthorized|forbidden|401|403)"
```
...
### Example Alert Query
```logql
sum(rate({kubernetes_namespace="agent-platform"} |~ "(?i)error"[5m])) > 0.1
```
