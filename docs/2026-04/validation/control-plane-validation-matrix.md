...
| ID | Use case | Status | Owner | Validation command/procedure | Expected result | Evidence |
|---|---|---|---|---|---|---|
| H-01 | Hermes CodeRun receives synthetic bridge event | `PASS` | control-plane loop | `2026-05-04T07:28Z` after PR #4925 merged and ArgoCD synced `hermes-control-plane-builder` to `main`, ran RBAC prerequisite checks (`can-i` returned yes for secret/pod/service reads plus CodeRun/pod create/delete), then executed `PRESENCE_SHARED_TOKEN=<from cto/openclaw-discord-tokens> python3 scripts/presence-smoke-hermes-coderun.py --mode live` without printing the token. Smoke run `hermes-coderun-7fwzkv6n` applied a temporary Hermes `CodeRun`, observed route registration, posted a synthetic `cto.presence.v1` event through `/presence/inbound`, reached the script success path, deleted the CodeRun, and confirmed the route was absent afterward. | Adapter route registration and authenticated bridge `/presence/inbound` delivery accept a synthetic normalized event in-cluster; temporary resources clean up. | `docs/2026-04/validation/hermes-presence-coderun-live-2026-05-04.md`. Adapter pod/log consumption marker was not found before timeout, so stronger worker-consumption evidence is still required for live Discord/session rows. |
