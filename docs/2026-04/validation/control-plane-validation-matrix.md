...
| H-18 | Attachments reach Hermes input/inbox | `UNIT_PASS` | control-plane loop | `2026-05-03T23:15:44Z npm test` in `apps/hermes-presence-adapter` (`6/6` pass): adapter rejects malformed attachment metadata before Hermes input and existing Hermes input path appends attachment URLs into the input payload. `npm run build` passed. Bridge package `npm test` (`25/25`) also validates rich attachment contract shape. | Hermes adapter accepts only well-formed normalized attachments and forwards attachment context through the credential-free adapter path. | Live CodeRun/Discord attachment evidence still required. |
| H-19 | Replies reach Hermes input/inbox | `UNIT_PASS` | control-plane loop | `2026-05-04T00:22Z npm test` in `apps/hermes-presence-adapter` (`7/7` pass): `forwards Discord reply reference metadata to Hermes input`. `npm run build` passed. | Hermes input metadata includes `discord_reference_message_id`, `discord_reference_channel_id`, and `discord_reference_guild_id` from normalized inbound events. | Live CodeRun/Discord reply evidence still required. |
