...
| Hermes ingress/outbound/session | `UNIT_PASS` | Hermes adapter unit coverage validates authenticated inbound delivery, metadata validation, deterministic session/home/route metadata forwarding, and outbound status intent posting. A post-merge live Hermes CodeRun smoke now proves route registration plus synthetic `/presence/inbound` HTTP `202` delivery and cleanup; adapter-log/worker-consumption proof and live Discord ingress/outbound evidence remain before the area can move to `PASS`. |
