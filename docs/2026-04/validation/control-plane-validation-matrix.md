...
| DS-08 | Empty-text message is represented | `NOT_STARTED` | TBD | Send message with attachment/embed and no text. | Event preserves empty text and non-text metadata instead of dropping message. | TBD |
| DS-09 | Attachment metadata is represented | `NOT_STARTED` | TBD | Send controlled file/image attachment. | Event includes attachment ID, filename, URL, content type/size/spoiler where available. | TBD |
| DS-10 | Reply/reference metadata is represented | `NOT_STARTED` | TBD | Reply to a controlled Discord message. | Event includes reference/reply metadata sufficient for session continuation. | TBD |
| DS-11 | Mention/addressing metadata selects intended agent | `NOT_STARTED` | TBD | Mention canonical agent in shared channel. | Selected agent metadata is present; unrelated routes do not receive event. | TBD |
...
| ID | Use case | Status | Owner | Validation command/procedure | Expected result | Evidence |
|---|---|---|---|---|---|---|
| H-01 | Hermes CodeRun receives synthetic bridge event | `UNIT_PASS` | control-plane loop | `2026-05-03T21:14:12Z npm test` in `apps/discord-bridge` (`23/23` pass) and `apps/hermes-presence-adapter` (`5/5` pass): synthetic `/presence/discord-events` fanout plus authenticated adapter inbound/inbox fallback. | Adapter receives authenticated inbound and writes Hermes input or fallback inbox. | Real CodeRun smoke still required via `scripts/presence-smoke-hermes-coderun.py` live mode. |
...
| H-18 | Attachments reach Hermes input/inbox | `NOT_STARTED` | TBD | Send attachment to active Hermes route. | Hermes receives attachment metadata in input payload. | TBD |
| H-19 | Replies reach Hermes input/inbox | `NOT_STARTED` | TBD | Send reply/reference to active Hermes route. | Hermes receives reply metadata in input payload. | TBD |
