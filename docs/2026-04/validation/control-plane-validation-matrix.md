...
| Hermes ingress/outbound/session | `UNIT_PASS` | Hermes adapter unit coverage validates authenticated inbound delivery, metadata validation, deterministic session/home/route metadata forwarding, and outbound status intent posting. Post-merge live Hermes CodeRun smokes now prove route registration, synthetic `/presence/inbound` HTTP `202` delivery, adapter pod discovery/log-tail, and cleanup; live Discord ingress/outbound evidence and a semantic worker-response marker remain before the whole area can move to `PASS`. |
...
| Ops hardening | `UNIT_PASS` | Workflow audit confirms the existing Discord bridge publish workflow and Hermes adapter publish workflow are syntax-valid and merged. Discord bridge publish succeeded on `main` after PR #4925; Hermes adapter test/build succeeded but its main-branch publish job failed with GHCR `write_package` permission denial, so adapter image publish remains below `PASS`. Operator runbook/rollback skeleton and branch reconciliation handoff exist; builder RBAC is now synced/healthy and live smoke prerequisites are unblocked. Live scale/failure validation, adapter publish permission repair, and redacted diagnostics remain. |
...
| ID | Use case | Status | Owner | Validation command/procedure | Expected result | Evidence |
|---|---|---|---|---|---|---|
| H-01 | Hermes CodeRun receives synthetic bridge event | `PASS` | control-plane loop | Latest refresh `2026-05-04T12:14Z`: with local/remote `main` and ArgoCD `hermes-control-plane-builder` both at `0b8319b926fad82cf0cb9ca4a2d25dbe4c974d48`, RBAC prerequisite checks returned yes for secret-name/pod/service reads plus CodeRun/pod create/delete. Ran `PRESENCE_SHARED_TOKEN=*** python3 scripts/presence-smoke-hermes-coderun.py --mode live` without printing the token. Smoke run `hermes-coderun-59h5x1qa` applied a temporary Hermes `CodeRun`, observed route registration, posted a synthetic `cto.presence.v1` event through `/presence/inbound`, discovered/log-tailed adapter pod `t1-codex-gpt-5-codex-default-0a9743b0-v1-9t9jp`, deleted the CodeRun, and confirmed the route was absent afterward. Earlier 07:28Z/09:33Z/10:40Z runs provide repeated evidence. | Adapter route registration and authenticated bridge `/presence/inbound` delivery accept a synthetic normalized event in-cluster; adapter pod is observable; temporary resources clean up. | `docs/2026-04/validation/hermes-presence-coderun-live-2026-05-04.md`. Still not real Discord ingress/outbound or semantic worker response evidence, so H-02/H-03/H-04/H-20 remain separate. |
...
| OPS-02 | Hermes presence adapter image publish workflow verified | `UNIT_PASS` | control-plane loop | `2026-05-04T07:40Z` inspected merged `.github/workflows/hermes-presence-adapter-publish.yml` and `.github/actions/docker-build-push/action.yaml`, queried `gh run list --workflow hermes-presence-adapter-publish.yml`, and viewed failed main push run `25305273066` for merge commit `67c415654fa22ee27fc62dd72c649a518b280ca7`. Test/build job succeeded, but publish job failed while pushing `ghcr.io/5dlabs/hermes-presence-adapter:latest` with GHCR `permission_denied: write_package`. | Workflow is now on `main` and validates tests/builds, but final image publish requires package/workflow permission repair before `PASS`. | Run `25305273066`; no secrets printed. |
| OPS-03 | Branch reconciliation handoff exists before image/GitOps promotion | `PASS` | control-plane loop | `2026-05-04T07:28Z` verified PR #4925 merged at `2026-05-04T06:52:47Z` with merge commit `67c415654fa22ee27fc62dd72c649a518b280ca7`, created local backup branch `local-main-before-pr4925-sync-20260504T072823Z`, then reset local `main` to `origin/main` after confirming the working tree had no product diffs. | Divergent local stack is now merged/reconciled through PR review instead of direct local-main push/rebase surgery; local `main...origin/main` is clean except ignored/untracked `.hermes/`. | PR #4925: https://github.com/5dlabs/cto/pull/4925; backup branch `local-main-before-pr4925-sync-20260504T072823Z`; no secrets printed. |
| OPS-04 | ArgoCD apps healthy after rollout | `UNIT_PASS` | control-plane loop | `2026-05-04T07:28Z kubectl -n argocd get application hermes-control-plane-builder -o jsonpath=...` returned target revision `main`, sync `Synced`, health `Healthy`, and live revision `4a08272fe030a499c05b250d87f1f145942d522f`. | Builder GitOps application is synced/healthy after PR #4925 and the follow-up merge-authority doc commit. | Builder app only; broader app fleet rollout and production bridge image pin audit still required before final ops `PASS`. |
| OPS-17 | Builder RBAC prerequisite is self-contained for live smoke handoff | `PASS` | control-plane loop | `2026-05-04T07:28Z` after PR #4925 merge and ArgoCD sync, reran live prerequisite checks: `kubectl auth can-i get secrets/openclaw-discord-tokens -n cto`, `get pods -n cto`, `get services -n bots`, `create pods -n cto`, `create coderuns -n cto`, and `delete coderuns -n cto` all returned `yes`. Safe resource-name reads found `openclaw-discord-tokens` and `discord-bridge-http`; then the live Hermes CodeRun smoke created/deleted a temporary CodeRun and route without printing token values. | Builder service account now has the read/create/delete surface needed for redacted live smoke validation and no longer depends on the missing stale `cto-hermes-gateway` role. | `docs/2026-04/validation/hermes-presence-coderun-live-2026-05-04.md`; no secret values printed. |
