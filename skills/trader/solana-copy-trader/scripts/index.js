...
`.trim());
    ...
    // ═══════════════════════════════════════
      ...
      // Auto-shutdown after PAPER_TIMEOUT_MS env var (default: run forever)
      const timeoutMs = parseInt(process.env.PAPER_TIMEOUT_MS || '0');
      if (timeoutMs > 0) {
        console.log(`[CopyTrade] Auto-shutdown in ${(timeoutMs / 1000).toFixed(0)}s\n`);
        setTimeout(() => {
          copyBot.stop();
        }, timeoutMs);
      }
