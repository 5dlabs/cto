...
### MCP Tool Server
- **Health:** `http://tools.fra.5dlabs.ai/health`
- **MCP Endpoint:** `http://tools.fra.5dlabs.ai/mcp`
- **Protocol:** JSON-RPC 2.0 over HTTP POST

### OpenBao (Secrets Vault)
```bash
# Access via kubectl
kubectl exec -n openbao openbao-0 -- bao <command>

# Common commands
bao kv put secret/path key=value
bao kv get secret/path
bao kv list secret/
bao status
```
...
### Kubernetes
```bash
# CTO namespace operations
kubectl -n cto get pods
kubectl -n cto logs <pod>
kubectl -n cto exec -it <pod> -- sh
```

### TMux (Swarm Visibility)
```bash
# Create session
...
# Attach
tmux attach -t forge-swarm

# Send to pane
tmux send-keys -t forge-swarm:0.1 "command" Enter
```
...
```bash
curl -s http://tools.fra.5dlabs.ai/health | jq
```

### List All Tools
```bash
curl -s -X POST http://tools.fra.5dlabs.ai/mcp \
...
| File | Purpose |
|------|---------|
| `cto-config.json` | Agent tool assignments |
| `client-config.json` | Runtime tool config |
| `values.yaml` | Helm chart values |
| `ExternalSecret` | ESO sync definitions |

## Worktree Commands

```bash
# Switch to forge worktree
cd ~/.cursor/worktrees/cto/forge

# Check status
git status

# Create PR
gh pr create --title "feat(forge): description" --body "..."
```
