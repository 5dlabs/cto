# TOOLS.md - Stitch Development Tools

## CTO Repository

**Worktree path:** `/Users/jonathonfritz/clawd-stitch/cto`
...
| Component | Path |
|-----------|------|
| PM Server (Rust) | `crates/pm-server/` |
| Controller (Rust) | `crates/controller/` |
| Config types | `crates/config/src/types.rs` |
| Linear sync | `crates/linear-sync/` |
| CTO config | `config/cto-config.json` |
| Templates | `config/templates/` |
| Argo sensors | `infra/gitops/manifests/argo-workflows/sensors/` |
...
# Test endpoints
curl -X POST http://localhost:3000/webhooks/github/comment \
...
# Get sensor details
kubectl describe sensor stitch-pr-review -n automation
```

### GitHub CLI

```bash
# List Stitch-related PRs
gh pr list --state all --search "stitch" --repo 5dlabs/cto

# View PR details
gh pr view <number> --repo 5dlabs/cto

# Check app installations
gh api /app/installations

# Test webhooks
gh api repos/5dlabs/cto/hooks
```
...
| Secret | Path |
|--------|------|
| GitHub App Stitch | `op://Development/GitHub-App-Stitch/...` |
| Linear App Stitch | `op://Development/Linear-App-Stitch/...` |
| PM Webhook Secret | `op://Automation/pm-webhook-secret/...` |
...
**Before committing:**
```bash
cargo fmt && cargo clippy --all-targets -- -W clippy::pedantic && cargo test
```
...
- Plan: `docs/stitch-mentions-and-remediation-plan.md`
- Agent spec: `.codex/agents/stitch-reviewer.md`
- Sensor: `infra/gitops/manifests/argo-workflows/sensors/stitch-pr-review-sensor.yaml`

## Relevant Skills

### Code Review Skills
- `stitch-review` - Stitch code review workflow and GitHub App integration
- `pr-review` - Pull request review patterns (focus areas, feedback, approval)
- `code-review` - Code quality review patterns
- `differential-review` - Security-focused PR/diff review
- `receiving-code-review` - How to handle review feedback
- `requesting-code-review` - How to request effective reviews

### Quality Skills
- `code-maturity` - 9-category codebase assessment
- `security-analysis` - OWASP Top 10, secret scanning
- `testing-strategies` - Unit, integration, E2E patterns

### Rust Skills
- `rust-patterns` - Axum, Tokio, error handling
- `rust-error-handling` - anyhow/thiserror patterns

### CTO Platform Skills
- `cto-platform` - Play workflow orchestration
- `argo-events` - Webhook handling and sensor configuration
- `controller-crd` - CRD processing and template system
- `healer` - Detection patterns and remediation
...
| `stitch-reviewer` | Code review specialist |
| `grizz-expert` | Go implementation help |
| `rex-expert` | Rust implementation help |
