...
if [[ "$RESULT_LC" == *"fail"* || "$RESULT_LC" == *"error"* ]]; then
  LEVEL="error"
fi
...
if [[ -n "$LV_BIN" ]]; then
  lv_args=(gate "$WORKFLOW" "$GATE" "$RESULT" --level "$LEVEL")
  ...
  echo "workflow-gate-audio: lobster-voice failed, falling back" >&2
fi
...
"$ROOT/intake/scripts/coordinator-speak.sh" "$ROLE_LABEL. $WORKFLOW gate $GATE result: $RESULT$CTX" "$LEVEL" || true
