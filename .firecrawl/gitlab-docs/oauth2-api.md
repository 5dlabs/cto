...
## Cross-origin resource sharing [Permalink](https://docs.gitlab.com/api/oauth2/\#cross-origin-resource-sharing "Permalink")
...
## Supported OAuth 2.0 flows [Permalink](https://docs.gitlab.com/api/oauth2/\#supported-oauth-20-flows "Permalink")
...
`application` credentials: _Application ID_ and _Client Secret_. The _Client Secret_ **must be kept secure**. It is also advantageous to keep the _Application ID_
...
### Prevent CSRF attacks [Permalink](https://docs.gitlab.com/api/oauth2/\#prevent-csrf-attacks "Permalink")
...
[CSRF attacks](https://wiki.owasp.org/index.php/Cross-Site_Request_Forgery_%28CSRF%29).

### Use HTTPS in production [Permalink](https://docs.gitlab.com/api/oauth2/\#use-https-in-production "Permalink")
...
### Authorization code with Proof Key for Code Exchange (PKCE) [Permalink](https://docs.gitlab.com/api/oauth2/\#authorization-code-with-proof-key-for-code-exchange-pkce "Permalink")
...
redirected back to the specified `REDIRECT_URI`. The [scope parameter](https://docs.gitlab.com/integration/oauth_provider/#view-all-authorized-applications)
...
should be used when [SAML SSO](https://docs.gitlab.com/user/group/saml_sso/) is configured for the associated group.
...
2. With the authorization `code` returned from the previous request (denoted as
...
3. To retrieve a new `access_token`, use the `refresh_token` parameter. Refresh tokens may
...
### Authorization code flow [Permalink](https://docs.gitlab.com/api/oauth2/\#authorization-code-flow "Permalink")
...
[authorization code flow with PKCE](https://docs.gitlab.com/api/oauth2/#authorization-code-with-proof-key-for-code-exchange-pkce),
...
2. With the authorization `code` returned from the previous request (shown as
`RETURNED_CODE` in the following example), you can request an `access_token`, with
any HTTP client. The following example uses Ruby’s `rest-client`:
...
### Device authorization grant flow [Permalink](https://docs.gitlab.com/api/oauth2/\#device-authorization-grant-flow "Permalink")
...
2. The device client displays the `user_code` and `verification_uri` from the response to the
...
3. Immediately after displaying the `verification_uri` and `user_code`, the device client
...
4. The device client receives a response from the token endpoint. If the authorization was successful,
...
A sample application that implements the client side device flow can be found at: [https://gitlab.com/johnwparent/git-auth-over-https](https://gitlab.com/johnwparent/git-auth-over-https).

### Resource owner password credentials flow [Permalink](https://docs.gitlab.com/api/oauth2/\#resource-owner-password-credentials-flow "Permalink")
...
[two-factor authentication](https://docs.gitlab.com/user/profile/account/two_factor_authentication/) turned on
and [enterprise users](https://docs.gitlab.com/user/enterprise_user/)
with [password authentication disabled for their group](https://docs.gitlab.com/user/enterprise_user/#restrict-authentication-methods).
These users can access the API using [personal access tokens](https://docs.gitlab.com/user/profile/personal_access_tokens/)
instead.

Ensure the [**Allow password authentication for Git over HTTP(S)**](https://docs.gitlab.com/administration/settings/sign_in_restrictions/#allow-password-authentication-for-git-over-https)
...
[personal access tokens](https://docs.gitlab.com/user/profile/personal_access_tokens/) are a better
choice.
...
## Access GitLab API with `access token` [Permalink](https://docs.gitlab.com/api/oauth2/\#access-gitlab-api-with-access-token "Permalink")
...
## Access Git over HTTPS with `access token` [Permalink](https://docs.gitlab.com/api/oauth2/\#access-git-over-https-with-access-token "Permalink")

A token with [scope](https://docs.gitlab.com/integration/oauth_provider/#view-all-authorized-applications)`read_repository` or `write_repository` can access Git over HTTPS. Use the token as the password.
...
Alternatively, you can use a [Git credential helper](https://docs.gitlab.com/user/profile/account/two_factor_authentication/#oauth-credential-helpers)
to authenticate to GitLab with OAuth. This handles OAuth token refresh
automatically.

## Retrieve the token information [Permalink](https://docs.gitlab.com/api/oauth2/\#retrieve-the-token-information "Permalink")
...
### Deprecated fields [Permalink](https://docs.gitlab.com/api/oauth2/\#deprecated-fields "Permalink")
...
## Revoke a token [Permalink](https://docs.gitlab.com/api/oauth2/\#revoke-a-token "Permalink")
...
## OAuth 2.0 tokens and GitLab registries [Permalink](https://docs.gitlab.com/api/oauth2/\#oauth-20-tokens-and-gitlab-registries "Permalink")
...
the [container registry API](https://docs.gitlab.com/api/container_registry/).
