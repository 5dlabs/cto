...
WORKFLOW_NAME=$(kubectl create -f test-project/test-project-workflow.yaml -o jsonpath='{.metadata.name}' 2>/dev/null || echo "")

if [ -z "$WORKFLOW_NAME" ]; then
    echo "❌ Failed to create workflow"
    echo "   Make sure test-project/test-project-workflow.yaml exists"
    exit 1
fi
