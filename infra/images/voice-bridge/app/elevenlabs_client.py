...
class ElevenLabsClient:
    ...
    async def transcribe(self, audio_bytes: bytes) -> str:
        """ElevenLabs Scribe STT. TODO: switch to real multipart upload once
        we confirm the audio container (opus-webm vs pcm16 wav).
        """
        ...
        if not audio_bytes:
            return ""
        # TODO: real call to https://api.elevenlabs.io/v1/speech-to-text
        # with `multipart/form-data` once audio container is decided.
        log.info("transcribe() stub: %d bytes", len(audio_bytes))
        return ""
