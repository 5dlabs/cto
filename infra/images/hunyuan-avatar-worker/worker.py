"""Hunyuan Avatar NATS worker, consumes render requests and publishes results."""
...
RENDER_REQUESTS = Counter("avatar_render_requests_total", "Total render requests", ["persona_id", "status"])
RENDER_DURATION = Histogram("avatar_render_duration_seconds", "Render duration", buckets=[1, 5, 10, 30, 60, 120, 300, 600, 1800])
GPU_MEMORY = Gauge("avatar_gpu_memory_bytes", "GPU memory used")
QUEUE_DEPTH = Gauge("avatar_queue_pending", "Pending messages in queue")
...
async def run_worker():
    ...
    consumer_name = os.environ.get("NATS_CONSUMER", "hunyuan-avatar-worker")
    queue = os.environ.get("NATS_QUEUE", "avatar-workers")
    ack_wait_str = os.environ.get("NATS_ACK_WAIT", "30m")
    ...
    log.info("Pre-loading Hunyuan avatar model...")
