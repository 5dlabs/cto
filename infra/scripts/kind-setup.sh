...
echo "      docker build -t registry.5dlabs.ai/5dlabs/controller:dev -f infra/images/controller/Dockerfile ."
echo "      kind load docker-image registry.5dlabs.ai/5dlabs/controller:dev --name ${CLUSTER_NAME}"
echo ""
echo "   2. Install controller with Helm:"
echo "      helm install cto infra/charts/cto \\"
echo "        --namespace cto \\"
echo "        --set image.tag=dev \\"
echo "        --set image.pullPolicy=Never"
echo ""
echo "   3. Or deploy individual components:"
echo "      helm template cto infra/charts/cto | kubectl apply -f -"
echo ""
