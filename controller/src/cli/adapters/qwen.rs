//! Qwen Adapter Implementation (Stub)
...
#[derive(Debug)]
pub struct QwenAdapter { base: BaseAdapter }

impl QwenAdapter {
    pub async fn new() -> Result<Self, AdapterError> {
        let base = BaseAdapter::new(CLIType::Qwen);
        warn!("QwenAdapter is a stub implementation");
        Ok(Self { base })
    }
}
...
impl CliAdapter for QwenAdapter {
    ...
    fn get_executable_name(&self) -> &str { "qwen" }
