...
//!
//! This module provides comprehensive security controls for the Agent Remediation Loop,
...
impl SecurityManager {
    ...
    pub async fn validate_operation(
        ...
        if let Err(e) = self.check_rate_limit(context).await {
            ...
            self.audit_security_event(context, "rate_limit_exceeded", false, Some(&e.to_string())).await?;
        } else {
        ...
        if let Some(input) = input_data {
            match self.validate_input(context, input).await {
                Ok(validation_result) => {
                    ...
                    metadata.insert("input_validation".to_string(),
        ...
        if let Err(e) = self.validate_github_token(context).await {
            ...
            self.audit_security_event(context, "token_validation_failed", false, Some(&e.to_string())).await?;
        } else {
        ...
        if let Err(e) = self.validate_rbac(context).await {
            ...
            self.audit_security_event(context, "rbac_denied", false, Some(&e.to_string())).await?;
        } else {
        ...
        if errors.is_empty() {
            self.audit_security_event(context, "security_validation_passed", true, None).await?;
        }
        ...
        if let Some(user) = &context.user {
            if !self.authorized_users.contains(user) {
                return Err(SecurityError::AuthorizationError(
                    format!("User {} is not authorized to perform this operation", user)
                ));
            }
        }
    ...
    async fn validate_github_token(&self, context: &SecurityContext) -> SecurityResult<()> {
        ...
        if matches!(context.operation.as_str(),
    ...
    async fn validate_rbac(&self, context: &SecurityContext) -> SecurityResult<()> {
        ...
        if matches!(context.operation.as_str(),
            ...
            self.rbac_validator.validate_permissions(&context.operation).await?;
        }
        Ok(())
    }
    ...
    async fn audit_security_event(
        ...
        let audit_event = audit::AuditEvent {
            ...
            resource: context.task_id.clone().unwrap_or_else(|| "unknown".to_string()),
            success,
            severity: if success { audit::AuditSeverity::Info } else { audit::AuditSeverity::Warning },
