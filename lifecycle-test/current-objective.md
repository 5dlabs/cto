...
- linear-sidecar-running: `kubectl get pods -n cto -o json | jq -e '[.items[] | select(.spec.containers | map(select(.name == "linear-sidecar")) | length > 0) | select(.status.phase == "Running" or .status.phase == "Pending")] | length > 0'`
- intake-succeeded: `kubectl get pods -n cto -o json | jq -e '[.items[] | select(.metadata.labels["cto.5dlabs.io/run-type"] == "intake" or .metadata.name | contains("intake")) | select(.status.phase == "Succeeded")] | length > 0'`
...
- cto-config-configmap-exists: `kubectl get configmap -n cto -o json | jq -e '[.items[] | select(.metadata.name | startswith("cto-config-project-"))] | length > 0'`
...
- intake-pr-created: `gh pr list --repo 5dlabs/prd-prd-alerthub-e2e-test --state all --json title,headRefName --jq '[.[] | select(.headRefName | startswith("intake-"))] | length > 0'`
